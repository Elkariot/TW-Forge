...
// ProjectileWriter persists projectile changes. Implemented by writer.GameWriter,
// shared with the RTW/M2TW Writer and M2TWWriter interfaces.
type ProjectileWriter interface {
	SaveProjectilesDraft(file domain.ProjectileFile, changes map[string]ChangeType) error
	Apply() error
}

// ProjectileRepository manages descr_projectile.txt entries. Its shape mirrors
// GameRepository's unit handling (in-memory working copy + change tracking +
// draft/apply persistence), but it is game-version agnostic: RTW and M2TW share
...
type InMemoryProjectileRepository struct {
	...
	writer   ProjectileWriter
	changes  map[string]ChangeType
}

func NewProjectile(data domain.ProjectileFile, writer ProjectileWriter) *InMemoryProjectileRepository {
...
func (r *InMemoryProjectileRepository) SaveDraft() error {
	if r.writer == nil {
		return fmt.Errorf("writer not configured")
	}
	return r.writer.SaveProjectilesDraft(r.working, r.changes)
}

func (r *InMemoryProjectileRepository) Save() error {
	if r.writer == nil {
		return fmt.Errorf("writer not configured")
	}
