...
import {
  ...
  CopyM2TWUnit, SaveProjectiles, ProjectilesHaveUnsavedChanges,
...
async function checkChanges() {
  const [unitChanges, projChanges] = await Promise.all([
    isM2TW.value ? M2TWHasUnsavedChanges() : HasUnsavedChanges(),
    ProjectilesHaveUnsavedChanges(),
  ])
  hasChanges.value = unitChanges || projChanges
}
