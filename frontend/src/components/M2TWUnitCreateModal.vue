...
onMounted(async () => {
  try {
    allUnits.value = await GetM2TWAllUnits()
...
function done() { emit('created', createdType.value) }
function onSaved() { emit('created', createdType.value) }
function onDeleted() { emit('close') }
</script>
...
.modal {
  background: var(--color-cell); border: 1px solid var(--color-border-soft); border-radius: 10px;
  display: flex; flex-direction: column; width: 420px; max-height: 90vh; overflow: hidden;
  box-shadow: 0 24px 60px rgba(0,0,0,0.5);
}
.modal--wide { width: 92vw; height: 90vh; }
.modal-header {
...
.btn-done { background: var(--color-primary); color: var(--color-text); border: none; border-radius: 4px; padding: 5px 14px; font-size: 12px; cursor: pointer; }
.btn-done:hover { background: var(--color-primary-hover); }
.modal-close { background: none; border: none; color: var(--color-text-muted); font-size: 16px; cursor: pointer; padding: 2px 4px; border-radius: 3px; }
.modal-close:hover { color: var(--color-accent); background: rgba(149,232,225,0.2); }
.modal-body { display: flex; flex-direction: column; gap: 16px; padding: 20px 18px; }
.field { display: flex; flex-direction: column; gap: 5px; }
.field-label { font-size: 11px; color: var(--color-text-dim); }
.field-input {
  background: var(--color-input-bg); border: 1px solid var(--color-border-soft); border-radius: 4px;
  color: var(--color-text); font-size: 13px; padding: 6px 8px; width: 100%;
}
.field-input:focus { outline: none; border-color: var(--color-accent); }
.field-input option { background: var(--color-cell); }
.dict-preview { font-size: 11px; color: var(--color-text-muted); background: var(--color-input-bg); border-radius: 4px; padding: 5px 8px; margin-top: -8px; }
.dict-preview code { color: #3aba80; font-size: 11px; }
.modal-error { font-size: 12px; color: var(--color-error); }
.modal-actions { display: flex; gap: 8px; margin-top: 4px; }
.btn-create { background: var(--color-primary); color: var(--color-text); border: none; border-radius: 5px; padding: 8px 20px; font-size: 13px; cursor: pointer; }
.btn-create:disabled { opacity: 0.5; cursor: default; }
.btn-create:not(:disabled):hover { background: var(--color-primary-hover); }
.btn-cancel { background: transparent; border: 1px solid var(--color-border-soft); color: var(--color-text-dim); border-radius: 5px; padding: 8px 16px; font-size: 13px; cursor: pointer; }
.btn-cancel:hover { border-color: var(--color-text-dim); color: var(--color-text-dim); }
.modal-edit { flex: 1; overflow: hidden; display: flex; flex-direction: column; }
</style>
